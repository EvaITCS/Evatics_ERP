...
module "backend_service" {
 ...
 secrets = [
    ...
    { name = "SPRING_MAIL_USERNAME", value_from = module.secrets.mail_username_secret_arn },
    { name = "SPRING_MAIL_PASSWORD", value_from = module.secrets.mail_password_secret_arn },

    { name = "GEOAPIFY_API_KEY", value_from = module.secrets.geoapify_api_key_secret_arn },
]

  enable_autoscaling       = var.backend_enable_autoscaling
  autoscaling_min_capacity = var.backend_desired_count
  autoscaling_max_capacity = var.backend_desired_count + 2
}
